<template>
  <div class="table" :style="{ 'text-shadow': theme ? '1px 1px 1px #000' : 'unset'}">
    <div class="row">
      <div class="number">
        {{ headRowTitle }}
      </div>
...
export default {
  ...
  methods: {
    setCanvasSize() {
      const teamsList = document.querySelectorAll('.row');
      if (teamsList.length > 1) {
        const width = teamsList[1].offsetWidth;
        const height = teamsList[1].offsetHeight - 8;

        const canvasElements = document.querySelectorAll('.particles-js-canvas-el');
        for (let i = 0; i < canvasElements.length; ++i) {
          canvasElements[i].setAttribute('width', width);
          canvasElements[i].setAttribute('height', height);
        }

        const efs = document.querySelectorAll('.effects-fb');
        for (let i = 0; i < efs.length; ++i) {
          efs[i].style = "width: " + width + "px;" + "height:" + height + "px;";
        }
